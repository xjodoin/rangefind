...
- Pasted envelope addresses resolve in the canonical address authority lane
  in both English and French. "311 A Bd Cartier O, Laval, QC H7N 2J3"
  returned zero results: the authority lane's single sorted-token key had no
  reading for the abbreviated street type ("Bd"), the directional letter
  ("O" = Ouest), the detached unit letter, or the trailing province + postal
  tail, and the text fallback's minShouldMatch tolerates only one unmatched
  term — the same failure hit English forms ("1500 Saint Clair Ave W" vs an
  indexed "St. Clair Avenue West", "#2F"/"Apt 3" units, "Ohio" vs "OH").
  "boul" and "bd" now normalize to the same canonical token as "boulevard"
  (shared with the builder), and address queries probe every plausible
  reading as extra authority keys (`addressAuthorityQueryKeys`): ambiguous
  abbreviation readings generated from libpostal's address dictionaries for
  14 languages (en, fr, es, de, pt, it, nl, ca, sv, da, nb, pl, cs, ro —
  "St" reads as Saint, Straße, Strada, or Straat; "Vle" as Viale; "Avda" as
  Avinguda; "Ul" as Ulica) plus curated saints, unit synonyms, French
  directionals, and single-token state/province names
  (`src/address_abbreviations.js`, regenerated by
  `scripts/generate_address_abbreviations.mjs`, adversarial cases ported
  from libpostal's expansion tests); Germanic/Scandinavian concatenated
  street suffixes merged and rewritten ("Marktstr 5", "Markt Str 5", and
  ...
  whole fix works against already-published indexes without a rebuild —
  verified live on the quebec, ontario, new-york, and berlin planet shards.
  Interpolated ranges probe the same readings through
