...
test("builder output is searchable through the range-based runtime", async (t) => {
  ...
  assert.match(manifest.docs.pointers.file, /^docs\/pointers\/0000\.[0-9a-f]{24}\.bin\.gz$/u);
  assert.match(manifest.docs.pages.pointers.file, /^docs\/pages\/0000\.[0-9a-f]{24}\.bin\.gz$/u);
  assert.match(manifest.docs.pointers.pack_table[0], /^0000\.[0-9a-f]{24}\.bin\.gz$/u);
  assert.match(manifest.docs.pages.pointers.pack_table[0], /^0000\.[0-9a-f]{24}\.bin\.gz$/u);
  assert.match(manifest.object_store.pack_table.docs[0], /^0000\.[0-9a-f]{24}\.bin\.gz$/u);
  assert.match(manifest.object_store.pack_table.docPages[0], /^0000\.[0-9a-f]{24}\.bin\.gz$/u);
  assert.match(manifest.object_store.pack_table.postingBlocks[0], /^0000\.[0-9a-f]{24}\.bin\.gz$/u);
  ...
  assert.match(manifest.doc_values.fields.tags.chunks[0].pack, /^0000\.[0-9a-f]{24}\.bin\.gz$/u);
  ...
  assert.match(manifest.doc_value_sorted.pack_table[0], /^0000\.[0-9a-f]{24}\.bin\.gz$/u);
  ...
  assert.match(manifest.facet_dictionaries.directory.pack_table[0], /^0000\.[0-9a-f]{24}\.bin\.gz$/u);
...
test("builder can reduce posting partitions in worker-owned packs", async (t) => {
  ...
  assert.ok(manifest.object_store.pack_table.terms.every(file => /^\d{4}\.[0-9a-f]{24}\.bin\.gz$/u.test(file)));
  ...
  assert.ok(manifest.object_store.pack_table.postingBlocks.every(file => /^\d{4}\.[0-9a-f]{24}\.bin\.gz$/u.test(file)));
  assert.equal(manifest.stats.posting_segment_block_storage, "range-pack-v1");
