...
// src/address_abbreviations.js
var ADDRESS_TOKEN_READINGS = /* @__PURE__ */ new Map([
  ["ab", [["alberta"]]],
  ["abby", [["abbey"]]],
  ["ac", [["avenida", "calle"]]],
  ["acc", [["access"]]],
...
var CONCATENATED_STREET_SUFFIXES = /* @__PURE__ */ new Set(["allee", "brucke", "burgermeister", "damm", "gade", "gasse", "gata", "gatan", "gate", "gracht", "hof", "kreuz", "laan", "markt", "platz", "plein", "quelle", "rundwanderweg", "siedlung", "stiege", "straat", "strasse", "vag", "vagen", "veien", "vej", "weg", "wiese"]);
var CONCATENATED_SUFFIX_READINGS = /* @__PURE__ */ new Map([
  ["str", ["strasse", "straat"]]
]);
...
var CANADIAN_POSTAL_FSA = /^[abceghj-nprstvxy]\d[abceghj-nprstvwxyz]$/u;
var CANADIAN_POSTAL_LDU = /^\d[abceghj-nprstvwxyz]\d$/u;
...
function unitLetterVariants(tokens) {
  ...
  if (numberIndex >= 0 && /^[a-z]$/u.test(tokens[numberIndex + 1] || "")) {
    ...
    variants.push({
      tokens: tokens.map((token, index) => index === numberIndex ? `${token}${tokens[numberIndex + 1]}` : token).filter((_, index) => index !== numberIndex + 1),
      cost: 1
    });
  }
...
var UNIT_MARKER_TOKENS = /* @__PURE__ */ new Set(["unit", "apt", "ste", "app"]);
...
function unitShedVariants(tokens) {
  const markerIndex = tokens.findIndex((token, index) => UNIT_MARKER_TOKENS.has(token) && /^\d+[a-z]?$/u.test(tokens[index + 1] || ""));
  if (markerIndex >= 0) {
  ...
  if (letteredIndex >= 0 && tokens.length > 2 && tokens.some(isNumberToken)) {
...
function tailVariants(tokens) {
  ...
  for (let branch = 0; branch < branches.length; branch++) {
    ...
    const postalIndex = trimmed.findIndex((token, index) => CANADIAN_POSTAL_FSA.test(token) && CANADIAN_POSTAL_LDU.test(trimmed[index + 1] || ""));
    if (postalIndex >= 0) {
      push(trimmed.filter((_, index) => index !== postalIndex && index !== postalIndex + 1));
    } else if (trimmed.length > 1 && /^\d{5}$/u.test(trimmed.at(-1)) && trimmed.slice(0, -1).some(isNumberToken)) {
...
function addressQueryTokenVariants(value, maxVariants = 40) {
...
function addressAuthorityQueryKeys(value) {
  const keys = [];
  const seen = /* @__PURE__ */ new Set();
