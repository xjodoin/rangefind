...
function matchesReverseFilters(result, resultTypes, locationTypes) {
  return (!resultTypes.size || result.types?.some((type) => resultTypes.has(type))) && (!locationTypes.size || locationTypes.has(result.locationType));
}
...
async function reverseGeocodeOsm(engine, rawParams = {}) {
  ...
  const results = uniqueReverseAddressResults((response.results || []).map(reverseAddressResult).filter((result) => matchesReverseFilters(result, resultTypes, locationTypes)));
  const localityRequested = (!resultTypes.size || [...resultTypes].some((type) => LOCALITY_TYPES.has(type) || type === "locality")) && (!locationTypes.size || locationTypes.has("APPROXIMATE"));
  if (!results.length && localityRequested) {
    ...
    const localityCandidates = uniqueReverseAddressResults((localityResponse.results || []).filter((result) => LOCALITY_TYPES.has(result.type)).map(reverseLocalityResult).filter((result) => matchesReverseFilters(result, resultTypes, locationTypes)));
