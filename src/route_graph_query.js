...
export async function openRouteGraph(options) {
  ...
  async function searchLazyTopGraph(bucket, fetched, forwardSeeds, backwardSeeds, factors, penalized, live) {
    const INF = Infinity;
  ...
  function searchQueryGraph(bucket, contexts, fetched, forwardSeeds, backwardSeeds, factors, penalized, live) {
    if (root.topOverlay.cells) {
      return searchLazyTopGraph(bucket, fetched, forwardSeeds, backwardSeeds, factors, penalized, live);
    }
    const INF = Infinity;
  ...
  async function route(params) {
    ...
    const primarySearch = await searchQueryGraph(bucket, contexts, fetched, forwardSeeds, backwardSeeds, factors, null, live);
    ...
    for (let i = 0; i < alternativeCount; i++) {
      const alternativeSearch = await searchQueryGraph(bucket, contexts, fetched, forwardSeeds, backwardSeeds, factors, penalized, live);
