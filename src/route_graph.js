...
const ROOT_VERSION = 6;
const CELL_VERSION = 8;
// Cell blocks are rebuild-only derived data. Root v6 alone retains v5 read
// compatibility because a rolling global catalog can temporarily reference
// both generations while country graphs publish independently.
...
export function decodeRouteTopSlice(bytes) {
  ...
  return { level, cellId, nodes, rowStart, targets, weights, isClique };
}
...
export function decodeRouteRoot(bytes) {
  ...
  const version = readVarint(bytes, state);
  if (version !== 5 && version !== ROOT_VERSION) throw new Error(`Unsupported route graph root version ${version}.`);
  ...
  const topMode = version >= 6 ? readVarint(bytes, state) : 0;
