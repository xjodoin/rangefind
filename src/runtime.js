...
export async function createSearch(options = {}) {
  ...
  // A caller-provided manifest object skips the fetch entirely — the sharded
  // layer uses this to run the root suggest-routing artifact through the
  // standard single-index authority machinery without a manifest file.
  ...
  function lexiconSuggestion(candidate) {
    ...
    if (candidate.rows?.length) {
      ...
      const ordinals = [];
      for (const [ordinal, weight] of candidate.rows) {
        if (weight !== top) break;
  ...
  // display whose normalized form equals `surface`'s, best rank first. Token
  // suffix keys share the normalized form, so callers that want whole-name
  // matches filter on `full`. Returns null when the index has no lexicon.
  // Root suggest-routing artifacts add `ordinals` (federation shard
  // provenance) to each match — the cheap locality-resolution path for
  // planet-scale sharded indexes.
  ...
  // Numeric relevance prior. A build-time score (manifest.rankPrior, or the
  // backwards-compatible manifest.linkGraph block), with field values in
  // [0, 1], is applied as a multiplicative document boost over the
  ...
  // index carries either prior block and the boost weight is positive.
  ...
  function rankPriorBoostWeight(params) {
    ...
    // Keep the public crawler controls working for existing indexes and apps.
    if (!manifest.rankPrior && (params.linkRank === false || params.linkRankBoost === 0)) return 0;
    const weight = Number(
      params.rankPriorBoost
      ?? (!manifest.rankPrior ? params.linkRankBoost : undefined)
      ?? cfg.boost
      ?? 0
    );
  ...
  async function applyRankPriorToResults(results, weight) {
    ...
    // A hydrated page declares every payload column in the manifest. When the
    // prior field is one of those columns, an absent property is the encoded
    // null/missing value and therefore rank zero; do not reopen doc-values to
    // rediscover that fact. This matters for sparse place corpora where civic
    // addresses intentionally carry no prominence score.
    ...
    for (const result of results) {
      ...
      const rank = typeof embedded === "number" && Number.isFinite(embedded)
        ? embedded
        : Number.isInteger(result.index) && store ? valueForDoc(store, field, result.index) : null;
  ...
  async function executeBoostedSearch(params, weight) {
    ...
    const factor = Math.max(1, Number(
      params.rankPriorOverfetch
      ?? (!manifest.rankPrior ? params.linkRankOverfetch : undefined)
      ?? cfg.overfetch
      ?? LINKRANK_OVERFETCH
    ));
  ...
  // Internal federation hook: resolve exact and interpolated addresses without
  // falling through to postings. Generational indexes must synthesize an
  // interpolation while its owning generation still has the compact range
  // geometry; an unhydrated row cannot be reconstructed by the outer merge.
...
async function createGenerationalSearch(root, options, baseUrl) {
  ...
  async function generationalAddressAuthoritySearch(params, { q, page, size, offset }) {
    ...
    const responses = await Promise.all(engines.map((engine, genIndex) => (
      typeof engine.searchAddressAuthority === "function" ? engine.searchAddressAuthority({
        ...params,
        includeResults: true,
        page: 1,
    ...
    for (let genIndex = 0; genIndex < responses.length; genIndex++) {
      const response = responses[genIndex];
      if (!response) continue;
      const target = response.stats?.plannerLane === "addressAuthorityExact"
        ? exact
        : response.stats?.plannerLane === "addressInterpolationExact"
          ? interpolated
          : null;
      if (!target) continue;
      for (const result of response.results || []) {
    ...
    // A concrete address always outranks a synthesized range point, matching
    // the single-index authority order. If a concrete row was tombstoned,
    // the live interpolation remains eligible.
    const hasInterpolationLane = responses.some(
      response => response?.stats?.plannerLane === "addressInterpolationExact"
    );
    ...
    const lane = exact.length || (!interpolated.length && !hasInterpolationLane)
      ? "addressAuthorityExact"
      : "addressInterpolationExact";
    const laneResponses = responses.filter(response => response?.stats?.plannerLane === lane);
    const complete = laneResponses.every(response => response.stats?.topKProven !== false);
    const response = {
...
async function createShardedSearch(root, options, baseUrl) {
  ...
  // Suggest routing: a root-level merged authority lexicon (suggest_routing.js)
  // answers autocomplete straight from the root — a couple of small range
  // reads instead of opening every shard's authority sidecar per keystroke.
  ...
  function suggestRoutingShards(ordinals) {
    ...
    const ids = [];
    for (const ordinal of ordinals) {
      const id = suggestRoutingShardIds[ordinal];
      if (id !== undefined && shardIdIndex.has(id) && !ids.includes(id)) ids.push(id);
    }
  ...
  async function suggest(params = {}) {
    ...
    if (suggestRouting && params.shards == null) {
      try {
        ...
        // The root artifact must never break autocomplete — fall through to
  ...
  // Exact-surface authority lookup against the root suggest-routing artifact:
  // matches carry `shards` (federation shard ids) so callers can scope a
  // follow-up search to the region that owns the name instead of fanning out.
  // Advisory by contract — returns null when the root has no artifact or the
  // lookup fails, and callers fall back to their unscoped path.
  async function authorityLookup(surface, params = {}) {
