...
export function writeRoadGraph(path, graph) {
  ...
  // Portal candidates are the one source column that can exceed V8's maximum
  // string length: India has tens of millions of id/coordinate triples. Keep
  // the rfroutesrc-v8 container but store those records as compact binary
  // columns. They occupy 16 bytes per candidate instead of a giant JSON
  // string and can be written through one small reusable buffer.
  const portalColumns = [];
